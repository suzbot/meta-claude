...
The promoted set of entries from the weekly intake scan. Decisions about candidates are logged in [`intake-decisions.md`](intake-decisions.md).
...
Sections, in order: **Principles and Methodologies → Examples and libraries → Latest Updates → Official Sources.** Within each section, recent additions appear first within their subtopic.

## Contents

- [Principles and Methodologies](#principles-and-methodologies)
  - [Spec, plan, and design](#spec-plan-and-design)
  - [Practitioner workflows](#practitioner-workflows)
  - [Context engineering](#context-engineering)
  - [Agents, orchestration, and harnesses](#agents-orchestration-and-harnesses)
  - [Memory and feedback loops](#memory-and-feedback-loops)
  - [Evaluation](#evaluation)
- [Examples and libraries](#examples-and-libraries)
  - [Curated indexes](#curated-indexes)
  - [Drop-in templates and harnesses](#drop-in-templates-and-harnesses)
- [Latest Updates](#latest-updates)
- [Official Sources](#official-sources)
  - [Capability references](#capability-references)
  - [Release/changelog tracking](#releasechangelog-tracking)
- [Quick-lookup by topic](#quick-lookup-by-topic)

---

## Principles and Methodologies

### Spec, plan, and design
...
**Reach for it when:** Introducing a new domain concept and wanting AI collaboration to stay grounded in shared terminology; pursuing *isomorphism* between code and domain — the discipline of keeping code structure aligned with domain structure, with names, relationships, and boundaries pulled from the domain rather than imposed by implementation convenience; evaluating whether ambiguity in requirements is causing downstream drift.
...
**Reach for it when:** Evaluating your alignment gates; deciding whether your upfront design is deep enough; catching yourself letting AI embed design decisions during implementation.
...
**Reach for it when:** Thinking about how to constrain AI-generated or AI-driven behavior inside a deterministic product, or conversely how to wrap deterministic mechanisms inside an AI-driven product — the framing transfers in either direction.
...
**Reach for it when:** Pushing back on "AI will design it for you"; evaluating whether your design discipline (anchor stories, TDD, spec-first review) keeps the what/how loop tight; checking whether abstractions in your code are emerging from real iteration or being imposed upfront.
...
**Reach for it when:** Defending isomorphism (code-domain alignment) when an AI proposes a too-clean-looking abstraction that drifts from the domain shape; deciding where to let AI drive (mechanical tasks, alternatives exploration) vs. stay in the driver's seat (abstraction design).
...
**Reach for it when:** Evaluating how context discipline scales — e.g., explicit grep-before-read rules, file-budget limits, chunked-read patterns; thinking about what belongs in CLAUDE.md vs. a skill vs. a spec.
...
**Reach for it when:** Justifying or improving step-spec / phase-design / decision-record documents; deciding what to persist vs. let live in conversation; onboarding a new team member to this discipline.
...
**Reach for it when:** Designing or auditing skill/command sets when ad-hoc commands are starting to feel like a loose toolkit; evaluating whether ad-hoc context files are reaching their useful limit; thinking about role-based agent separation.
...
### Agents, orchestration, and harnesses
...
### Drop-in templates and harnesses
...
Recent feature ships from Anthropic. Entries here move into Official Sources when durable, or get dropped when reference is no longer needed.
...
- **Abstractions, isomorphism, and code-domain alignment (DDD):** Fowler LLMs and Building Abstractions; Fowler What and How; Eric Evans; Schleicher (ubiquitous language).
...
- **Sub-agents:** Claudefa.st Sub-Agent Best Practices (dispatch patterns); VoltAgent awesome-claude-code-subagents (template collection).
- **Templates and scaffolding:** thedecipherist Starter Kit; VoltAgent awesome-claude-code-subagents (subagent-specific); everything-claude-code (full harness ecosystem); hesreallyhim awesome-claude-code (broader directory).
- **Long-running and durable agents:** Addy Long-running Agents (Ralph Loop, Brain/Hands/Session, Planner/Worker/Judge, Memory Bank).
- **Language specialist (Go):** JetBrains Go + Claude Code (version-aware idioms).
- **Autonomous / unattended workflows:** Claude Code Routines (Latest Updates).
